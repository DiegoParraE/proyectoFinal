const formContact = document.querySelector(".my-form");
  ...
  if (emailValidation.test(correo) && correo.length > 7) {
    if (nameValidator.test(nombre) && nombre.length > 10) {
      if (mensaje.length > 30) {
        ...
        console.log("no paso mensaje")
        document.querySelector('textarea[name="mensaje"]').classList.add("error");
      }
    } else {
      console.log("no paso nombre")
      document.querySelector('input[name="nombre"]').classList.add("error");
    }
  } else {
    console.log("no paso correo")
